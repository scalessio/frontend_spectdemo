...
Spectrum.prototype.createLabel = function(labelTech,start,colorStyle) {
    this.ctx.font = "bold 18px Verdana";
    this.ctx.fillStyle = colorStyle;
    this.ctx.fillText(labelTech, (start+5)/2, 15); 
}

Spectrum.prototype.addBoxPlot = function(transmissions,signals){
    for (let i = 0; i < transmissions.length; i++) { 
        var tx_1 = transmissions[i]    
        var start =  tx_1[0]
        var end = tx_1[1]
        var widt = (end-start)/2
        var selected__signal = signals[i]    
        var signalLabel = selected__signal[0]
        var colorLabel = 'green'

        
        this.ctx.lineWidth = 4;
        if(selected__signal[2] == 0){
            this.ctx.strokeStyle = 'green';
            this.createLabel(signalLabel,start,colorLabel)
        }else{
            colorLabel = 'red'
            this.ctx.strokeStyle = 'red';
            this.createLabel(signalLabel,start,colorLabel)
        }
        this.ctx.strokeRect(start/2, 90, widt, 280);
    ...
    /*for (let i = 0; i < transmissions.length; i++) { 
        console.log("Function spectrum: Print Transmission:")
        var start =  transmissions[i][0]
        var end = transmissions[i][1]
        ...
        this.ctx.strokeStyle = 'green';
        // larghezza, altezza
        this.ctx.moveTo(start/2, 90);
        this.ctx.lineTo(start/2, 300);
        this.ctx.stroke();
        ...
        this.ctx.strokeStyle = 'green';
        // larghezza, altezza
        this.ctx.moveTo(end/2, 90);
        this.ctx.lineTo(end/2, 300);
        this.ctx.stroke();*/
...
Spectrum.prototype.drawSpectrum = function(bins) {
    ...
    // FFT averaging -- Not use this code
    if (this.averaging > 0) {
...
function Spectrum(id, options) {
    ...
    this.canvas.width = 1395; // this is the half of processed data from the backend
    // we can use canvas style here
    // this.canvas.Style = 1395;
